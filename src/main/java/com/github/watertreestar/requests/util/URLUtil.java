...
import com.github.watertreestar.requests.Parameter;
...
public class URLUtil {
    public static URL concatUrl(URL url, Charset charset, List<Parameter<String>> params) {
        if (params.isEmpty()) {
            return url;
        }
    ...
    /**
     * 从parameter entry转换到{@link com.github.watertreestar.requests.Parameter}集合
     * @param params entry collection
     * @return
     */
    public static List<Parameter<String>> entryToParameter(Collection<? extends Map.Entry<String,?>> params) {
        List<Parameter<String>> parameterList = new ArrayList<>();
        for (Map.Entry<String,?> entry : params) {
            parameterList.add(Parameter.of(entry.getKey(), String.valueOf(entry.getValue())));
        }
